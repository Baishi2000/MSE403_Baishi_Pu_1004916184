...
def parity_plot(y_test, y_hat, ax, xlabel = 'actual value', ylabel = 'predicted value',
    ...
    lower = min(min(y_test), min(y_hat))
    upper = max(max(y_test), max(y_hat))

    r1 = np.arange(lower,upper,0.1)[:,None]
    r2 = r1
    r2value = round(r2_score(y_test, y_hat),3)
...
def pca_plot(X, n_comp = 3,  title = None, standardize = True, y_log = False, color_label = None, alpha = 0.3, return_scree = False):
    '''return the PCA plot of X, and return X in reduced dimensionality
    Input: 
    X: ndarray, no need standardized
    ...
    ax1.scatter(pca_dots[:,0], pca_dots[:,1], s = 6, c=color, marker='o', alpha=alpha)
    ax1.set_xlabel('PC1')
    ax1.set_ylabel('PC2')
    ax1.set_title(title)
    ...
    if return_scree:
        ...
        ax2.plot(np.arange(1, X.shape[1]+1, 1), pca_scree.explained_variance_ratio_, 'o-')
        ax2.set_xlabel('Principal Component')
        # set the ticks to be the number of principal components
        ax2.set_xticks(np.arange(1, X.shape[1]+1, 1))
        # turn on grid at the x axis, make grid only on integer ticks
...
def CrossValidate(estimator, X, y, task_type, splitter, cv_label = None, title = ''):
    ...
    CAN we modify this to a classfier model?'''
    # get the number of splits, tho already defined outside the function
...
from sklearn.linear_model import LogisticRegressionCV, LogisticRegression
def logistic_preserve(X, y, n_features, Cs = np.logspace(-5, 0, 100), C_opt = None, splitter = ShuffleSplit(
        n_splits=5, test_size=0.2, random_state=0)):
    '''Returns the index of features of top n_feature important
    Inputs:
    X: ndarray of full features
    y: target values
    n_features: the number of features to preserve
    Cs: a list of C values to try
    C_opt: the optimal C value, input manually
    splitter: splitter object which define the split of LassoCV

    Outputs:
    an ndarray of indexes where feature importance is preserved
    '''
    # Standardize the X using the entire X
    scaler = StandardScaler()
    X_std = scaler.fit_transform(X)

    if C_opt is None: # when we need lr_CV to find the optimal C
        lr_CV = LogisticRegressionCV(Cs = Cs,cv=splitter)
        lr_CV.fit(X_std, y)
        C_opt = lr_CV.C_[0]
        print('C_opt from CV ', C_opt, ' is used')
    else:
        print('Input C_opt ', C_opt, ' is used')

    # Fit LR with the optimal alpha value to the full training set
    ...
    return top_k_indices, feature_importances

'''ASSIGNMENT 9 FUNCTIONS'''

def SearchPara(estimator, X, y, parameters, task_type, splitter, cv_label = None, title = ''):
    '''plot a series of parity plots/Confusion Matrix/ROC curves from cross validation. Input data is raw and need not standardized.
    ...
    for i, (train_index, test_index) in enumerate(splitter.split(X, y, cv_label)): 
        ...
        if task_type == 'classification':
            # train error
            train_ax = axs[0, i]
            ConfusionMatrixDisplay.from_estimator(estimator, X_train_std, y_train, ax = train_ax, cmap = 'binary', colorbar = False)
            train_ax.set_title('Train Err, Split #'+ str(i+1))

            # validation error
            val_ax = axs[1, i]
            ConfusionMatrixDisplay.from_estimator(estimator, X_test_std, y_test, ax = val_ax, colorbar = False)
            val_ax.set_title('Val. Err, Split #'+ str(i+1))
